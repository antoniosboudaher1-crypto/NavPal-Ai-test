...
import { Clock, MapPin, Navigation, ArrowRight, ArrowLeft, ArrowUp, ChevronDown, ChevronUp, X, Flag, Timer, Ban, List } from 'lucide-react';
...
const RouteOverview: React.FC<RouteOverviewProps> = ({ route, isPreview, onClose, onStartNavigation, onStopNavigation }) => {
  ...
  return (
    <div className={`absolute top-20 right-6 z-30 transition-all duration-500 cubic-bezier(0.16, 1, 0.3, 1) ${isCollapsed ? 'w-auto' : 'w-full max-w-md'} animate-slide-in-right`}>
          ...
          <>
            ...
            >
               <div className="flex items-center gap-2 text-slate-400 group-hover:text-purple-400 transition-colors">
                 ...
                 <span className="text-xs font-bold uppercase tracking-wider">Route Details ({route.steps.length})</span>
               </div>
              ...
              <div className="relative animate-slide-down">
                ...
                <div className="overflow-y-auto max-h-[40vh] no-scrollbar p-2 space-y-1 bg-slate-950/30">
                  {route.steps.map((step, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex gap-4 p-3 rounded-xl hover:bg-white/5 transition-colors group relative overflow-visible">
                        
                        {/* Continuous Timeline Line */}
                        {!isLast && (
                          <div className="absolute left-[27px] top-8 bottom-[-14px] w-0.5 bg-slate-800 group-hover:bg-slate-700 transition-colors z-0"></div>
                        )}

                        {/* Step Number/Icon */}
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10 border-2 border-slate-900 shadow-md transition-transform group-hover:scale-110 ${idx === 0 ? 'bg-purple-600 text-white' : 'bg-slate-800 text-slate-400'}`}>
                          {getIconForManeuver(step.maneuver.type, step.maneuver.modifier)}
                        </div>

                        <div className="flex-1 min-w-0 z-10 pt-1">
                          <p className="text-slate-200 font-medium text-sm leading-snug">{step.instruction}</p>
                          ...
                          <div className="flex justify-between items-center mt-2">
                            ...
                            <div className="flex items-center gap-3 text-[10px] text-slate-500 font-bold tracking-wide">
                              <div className="flex items-center gap-1" title="Estimated time to destination from this step">
                  ...
                  <div className="flex gap-4 p-3 items-center pb-6">
                    ...
                    <span className="text-white font-bold text-sm">You have arrived</span>
                  </div>
                </div>
              </div>
            )}
